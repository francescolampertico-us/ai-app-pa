...
export default function MessagingMatrix() {
  ...
  const house = result?.message_house || {};
  ...
  const tabs = ['Message Map', ...Object.keys(result?.variants || {}).map((variant) => VARIANT_LABELS[variant] || variant)];
  const activeVariantKey = Object.keys(result?.variants || {}).find((key) => (VARIANT_LABELS[key] || key) === activeTab);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="p-10 max-w-6xl mx-auto relative z-10">
      <header className="page-header relative">
        ...
        <h1 className="display" style={{ fontSize: 42, color: '#fff', marginBottom: 10 }}>Messaging Matrix</h1>
        <p style={{ fontFamily: 'Inter', fontSize: 14, color: '#71717A', lineHeight: 1.65, maxWidth: '72ch', fontWeight: 300 }}>
          Builds a message house and platform-specific deliverables from a core policy position, optional proof points, supporting documents, and audience guidance.
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="glass-card p-8 flex flex-col gap-5">
          <div>
            ...
            <textarea value={position} onChange={(event) => setPosition(event.target.value)}
          ...
          <details className="rounded-xl border border-white/10 bg-black/20 p-5">
            ...
            <div className="mt-5 flex flex-col gap-5">
              <div>
                ...
                <textarea value={coreMessages} onChange={(event) => setCoreMessages(event.target.value)}
                  className="field resize-none" rows={4}
          ...
          <div>
            ...
            <textarea value={contextPaste} onChange={(event) => setContextPaste(event.target.value)}
        ...
        <div className="glass-card p-8 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
              {DELIVERABLES.map(([value, label]) => (
                <label key={value} className="flex items-center gap-2 text-sm text-slate-300">
                  <input type="checkbox" checked={selectedVariants.includes(value)} onChange={() => toggleVariant(value)} className="accent-violet-500" />
                  <span>{label}</span>
                </label>
              ))}
            </div>
          ...
          <button type="submit" disabled={loading || !position.trim() || selectedVariants.length === 0} className="btn-primary mt-auto">
            {loading ? <><SpinnerGap size={18} className="animate-spin" /> Generating…</> : <>Generate Messaging Matrix <ArrowRight size={18} /></>}
          </button>
        </div>
      </form>
          ...
          <div className="glass-card p-8">
            {activeTab === 'Message Map' && (
              <div className="space-y-6">
                ...
                <div>
                  <h2 className="display" style={{ fontSize: 26, color: '#fff', marginBottom: 12 }}>Overarching Message</h2>
            ...
            {activeTab !== 'Message Map' && (
              <pre className="whitespace-pre-wrap text-sm text-slate-300 leading-7">
                {activeVariantKey ? result.variants?.[activeVariantKey] : ''}
              </pre>
            )}
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button onClick={() => downloadText(job?.result_data?.markdown || '', 'messaging_matrix.md')}
            ...
            <button onClick={() => downloadArtifact(artifactMap['messaging_matrix.docx'])}
            ...
            <button onClick={() => downloadJson(result || {}, 'messaging_matrix.json')}
              className="flex items-center justify-center gap-2 py-4 px-6 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 transition-colors">
