import CitationButton from '../research/landing/CitationButton';

export default function ResearchPrototypeNote({
  category,
  message,
  secondaryLabel = 'Review Required',
  refs = [],
}) {
  return (
    <div
      className="mb-6 rounded-2xl border border-white/10 p-4"
      style={{ background: 'rgba(255,255,255,0.03)' }}
    >
      <div className="flex items-start justify-between gap-3 mb-2.5">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className="px-3 py-1 rounded-full text-[11px] uppercase tracking-[0.18em]"
            style={{ color: '#c4b5fd', background: 'rgba(109,40,217,0.18)', border: '1px solid rgba(109,40,217,0.28)' }}
          >
            {category}
          </span>
          <span
            className="px-3 py-1 rounded-full text-[11px] uppercase tracking-[0.18em]"
            style={{ color: '#f8fafc', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)' }}
          >
            {secondaryLabel}
          </span>
        </div>
        {refs.length > 0 && <CitationButton refs={refs} />}
      </div>
      <p className="app-copy">{message}</p>
    </div>
  );
}
