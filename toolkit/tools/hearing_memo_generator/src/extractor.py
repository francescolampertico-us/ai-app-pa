...
Two-pass LLM architecture:
  Pass 1 (gpt-4.1-mini): extraction — pulls structured data from the transcript
    in one or more chunk calls, then merges results.
  Pass 2 (gpt-4.1): polish — rewrites the prose fields (overview, opening
...
def _call_llm(user_prompt: str, model: str = None) -> dict:
    ...
    model = model or os.environ.get("MEMO_MODEL", "gpt-4.1-mini")
    ...
    try:
        ...
        error_msg = str(e)
        if ("rate_limit" in error_msg or "429" in error_msg) and model != "gpt-4.1-mini":
            print(f"Rate limited on {model}, falling back to gpt-4.1-mini...", file=sys.stderr)
            response = _openai_client.chat.completions.create(
                model="gpt-4.1-mini",
                messages=[
                    {"role": "system", "content": SYSTEM_PROMPT},
...
def _extract_via_llm(transcript: str, metadata_candidates: dict) -> dict:
    ...
    model = os.environ.get("MEMO_MODEL", "gpt-4.1-mini")
...
# ---------------------------------------------------------------------------
# Pass 2: prose polish via gpt-4.1
# ---------------------------------------------------------------------------
...
def _polish_extracted(llm_output: dict) -> dict:
    ...
    if not _openai_client:
        return llm_output
    try:
        print("  Polish pass (gpt-4.1)...", file=sys.stderr)
        response = _openai_client.chat.completions.create(
            model=POLISH_MODEL,
            messages=[
        ...
        polished = json.loads(response.choices[0].message.content.strip())
