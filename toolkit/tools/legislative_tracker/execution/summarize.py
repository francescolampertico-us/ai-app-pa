...
Two-pass approach for long bills:
  Pass 1 — gpt-4.1-mini extracts concrete facts from each chunk (cheap, reliable for extraction)
  Pass 2 — gpt-4.1 synthesizes extracted facts into the final structured summary (accurate, grounded)

Short bills (≤50K chars) go directly to gpt-4.1 in a single pass.
"""
...
# Bills under this threshold get single-pass summarization with gpt-4.1.
# Above this, we use two-pass: gpt-4.1-mini extraction → gpt-4.1 synthesis.
...
def summarize_bill(bill_detail: dict, bill_text: str, model: str = None) -> str:
    ...
    Short bills (≤50K chars): single-pass with gpt-4.1.
    Long bills: two-pass — gpt-4.1-mini extracts facts from chunks, gpt-4.1 synthesizes.

    Args:
        bill_detail: Normalized bill detail dict from LegiScanClient.get_bill().
        bill_text: Decoded full text of the bill.
        model: OpenAI model to use for synthesis (default: gpt-4.1).
...
def _two_pass_extract(client: OpenAI, bill_detail: dict, bill_text: str) -> str:
    ...
    Pass 1: gpt-4.1-mini extracts concrete facts from each chunk.
    Pass 2: Concatenated extractions become the input for the final gpt-4.1 synthesis.
    """
