...
COLUMNS = [
    ...
    ("Pitch Angle", 40),
    ("Why Now", 40),
    ("Relevance Confidence", 16),
    ("Topic Fit Score", 14),
    ("Supporting Evidence", 40),
    ("Matched Evidence Type", 20),
    ("Matched Title", 40),
    ("Matched URL", 40),
    ("Matched Summary", 40),
    ("Previous Story", 40),
...
def export_xlsx(result: dict, output_path: str):
    ...
    for row_idx, contact in enumerate(result.get("contacts", []), 2):
        ...
        row_data = [
            ...
            contact.get("show_name", ""),
            contact.get("host_name", ""),
            contact.get("host_contact_status", ""),
            ...
            contact.get("why_now", ""),
            contact.get("relevance_confidence", ""),
            contact.get("topic_fit_score", ""),
            contact.get("supporting_evidence", ""),
            contact.get("matched_evidence_type", ""),
            contact.get("matched_title", ""),
            contact.get("matched_url", ""),
            contact.get("matched_summary", ""),
            prev_title,
            prev_url,
            contact.get("email", ""),
            contact.get("notes", ""),
